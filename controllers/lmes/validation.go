...
func ValidateCustomArtifactValue(input string) error {
	...
	// Consider as plain text and validate for shell metacharacters
...
func ValidateTaskRecipes(args []lmesv1alpha1.TaskRecipe, argType string) error {
	for i, arg := range args {
		...
		if arg.Task != nil {
			if err := ValidateTaskName(*arg.Task); err != nil {
				return fmt.Errorf("%s[%d] task: %w", argType, i, err)
			}
		}
		...
		for j, metric := range arg.Metrics {
			if err := ValidateTaskName(metric); err != nil {
				return fmt.Errorf("%s[%d] metric[%d]: %w", argType, i, j, err)
			}
		}
...
func ValidateArgValue(value string) error {
	...
	// Additional check for shell metacharacters as defence in depth
...
func ContainsShellMetacharacters(input string) bool {
	// Shell metacharacters that can lead to command execution
